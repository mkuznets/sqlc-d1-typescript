export const MANAGED_SCENARIO_IDS = ["managed-d1/value-command-metadata", "managed-d1/macro-smoke", "managed-d1/batch-success", "managed-d1/batch-rollback", "managed-d1/direct-session", "managed-d1/bookmark-transfer", "managed-d1/native-error-identity", "managed-d1/post-execution-result-error"] as const;
export type ManagedScenarioId = typeof MANAGED_SCENARIO_IDS[number];

export interface ScenarioEnvironment { DB: D1Database; SCENARIO_AUTH_TOKEN?: string }
...
async function fixedBytes(value: string): Promise<Uint8Array> { return new Uint8Array(await crypto.subtle.digest("SHA-256", encoder.encode(value))); }
async function authorize(request: Request, expected: string | undefined): Promise<boolean> {
  ...
  const a = await fixedBytes(header.slice(7)), b = await fixedBytes(expected); let difference = 0;
...
export async function parseScenarioRequest(request: Request): Promise<ManagedScenarioId | null> {
  ...
  try {
    while (true) {
      ...
      length += value.byteLength; if (length > MAX_BODY_BYTES) { await reader.cancel(); return null; }
      chunks.push(value);
    }
  } finally { reader.releaseLock(); }
  const bytes = new Uint8Array(length); let offset = 0; for (const chunk of chunks) { bytes.set(chunk, offset); offset += chunk.byteLength; }
  let source: string; try { source = new TextDecoder("utf-8", { fatal: true }).decode(bytes); } catch { return null; }
  let value: unknown;
  try { value = JSON.parse(source); } catch { return null; }
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const keys = Object.keys(value as Record<string, unknown>);
  if (keys.length !== 1 || keys[0] !== "scenario") return null;
  return typeof (value as { scenario?: unknown }).scenario === "string" ? (value as { scenario: ManagedScenarioId }).scenario : null;
}
export function createScenarioHandler(registry: ScenarioRegistry) {
  return async (request: Request, env: ScenarioEnvironment): Promise<Response> => {
    ...
    try {
      ...
      const errorKind = error instanceof Error && error.name === "QueryResultError" ? "result" : error instanceof Error && error.name.endsWith("SqlcD1Error") ? "generated" : "native";
