...
import { createQuirk, createSample, getSampleById, listSamples, type CreateSampleArgs } from '../src/queries_sql';
...
describe('checked values against real D1', () => {
	...
	it('miniflare/conversion-physical-values - records the physical representations D1 actually returns', async () => {
		...
		const physical = await env.DB.prepare('SELECT * FROM samples WHERE id = ?').bind(inserted!.id).first<Record<string, unknown>>();
	...
	it('miniflare/error-result - fails closed when a stored value contradicts the generated type', async () => {
		...
		const insertContradiction = async (column: string, literal: string): Promise<number> => {
			...
			const result = await env.DB.prepare(`INSERT INTO samples (${columns.join(', ')}) VALUES (${values.join(', ')}) RETURNING id`).first<{
				id: number;
			}>();
			return result!.id;
		};
	...
	it('miniflare/error-batch-mapping - carries batchIndex, returns no partial tuple, and leaves batch writes committed', async () => {
		const db = new DB(env.DB);
		const failure = await caught(() => db.batch(createSample(completeArgs({ textValue: 'batch-survivor' })), createQuirk({ id: 2 })));
		expect(failure).toBeInstanceOf(QueryResultError);
		expect(failure).toMatchObject({
			operation: 'batch',
			queryName: 'CreateQuirk',
		...
		const survivors = await env.DB.prepare("SELECT count(*) AS total FROM samples WHERE text_value = 'batch-survivor'").first<{
			total: number;
		}>();
