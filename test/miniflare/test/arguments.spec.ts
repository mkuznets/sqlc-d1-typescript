...
const feedIds = async (userId: string): Promise<string[]> => {
  const rows = await env.DB.prepare("SELECT id FROM feeds WHERE user_id = ? ORDER BY id")
    .bind(userId)
    .all<{ id: string }>();
...
describe("argument model against real D1", () => {
  ...
  it("miniflare/argument-slice - a slice binds exactly its elements, for any length", async () => {
    ...
    for (const ids of [
      ["feed_slice1"],
      ["feed_slice1", "feed_slice3"],
      ["feed_slice1", "feed_slice2", "feed_slice4"],
    ]) {
    ...
    expect((await db.execute(listFeedsByIds({ ids: ["feed_slice2", "feed_slice2"] }))).map((row) => row.id)).toEqual([
      "feed_slice2",
    ]);
  ...
  it("miniflare/argument-empty-slice - an empty slice is rejected before any statement reaches D1", async () => {
    ...
    const untouched = await env.DB.prepare("SELECT updated_at FROM feeds WHERE id = 'feed_empty1'").first<{
      updated_at: number;
    }>();
  ...
  it("miniflare/argument-slice-commands - expands a slice for every ordinary command", async () => {
    ...
    // :execrows
    expect(
      await db.execute(deleteFeedsByIdsForUser({ userId: "user_cmd", ids: ["feed_cmd1", "feed_cmd2", "feed_cmd3"] })),
    ).toBe(3);
  });
