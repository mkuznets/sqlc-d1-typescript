...
const feedIds = async (userId: string): Promise<string[]> => {
	const rows = await env.DB
		.prepare("SELECT id FROM feeds WHERE user_id = ? ORDER BY id")
		.bind(userId)
		.all<{ id: string }>();
...
describe("argument model against real D1", () => {
	...
	it("miniflare/argument-slice - a slice binds exactly its elements, for any length", async () => {
		...
		expect((await db.execute(listFeedsByIds({ ids: ["feed_slice2", "feed_slice2"] }))).map((row) => row.id))
			.toEqual(["feed_slice2"]);
	...
	it("miniflare/argument-bind-order - binds a positional argument before the slice it precedes in the text", async () => {
		...
		const removed = await db.execute(deleteFeedsByIdsForUser({
			userId: "user_order",
	...
	it("miniflare/argument-empty-slice - an empty slice is rejected before any statement reaches D1", async () => {
		...
		expect(() => deleteFeedsByIdsForUser({ userId: "user_empty", ids: [] }))
			.toThrowError(QueryArgumentError);
		try {
		...
		await expect(async () => db.batch(
			touchFeedsByIds({ updatedAt: now + 1, ids: ["feed_empty1"] }),
			deleteFeedsByIdsForUser({ userId: "user_empty", ids: [] }),
		)).rejects.toThrowError(QueryArgumentError);
		const untouched = await env.DB
			.prepare("SELECT updated_at FROM feeds WHERE id = 'feed_empty1'")
	...
	it("miniflare/argument-slice-commands - expands a slice for every ordinary command", async () => {
		...
		// :many
		expect((await db.execute(listFeedsByIds({ ids: ["feed_cmd1", "feed_cmd3"] }))).map((row) => row.id))
			.toEqual(["feed_cmd1", "feed_cmd3"]);
		// :exec
		expect(await db.execute(touchFeedsByIds({ updatedAt: now + 5, ids: ["feed_cmd1", "feed_cmd2"] }))).toBeUndefined();
		const touched = await env.DB
			.prepare("SELECT id FROM feeds WHERE updated_at = ? ORDER BY id")
			.bind(now + 5)
			.all<{ id: string }>();
		expect(touched.results.map((row) => row.id)).toEqual(["feed_cmd1", "feed_cmd2"]);
		// :execlastid
		const copied = await db.execute(copySampleForTextValues({
			textValue: "cmd-sample-copy",
			values: ["cmd-sample-a", "cmd-sample-b"],
		}));
		expect(Number.isSafeInteger(copied)).toBe(true);
		const copiedRow = await env.DB
			.prepare("SELECT text_value, int_value FROM samples WHERE id = ?")
			.bind(copied)
		...
		// :execrows
		expect(await db.execute(deleteFeedsByIdsForUser({ userId: "user_cmd", ids: ["feed_cmd1", "feed_cmd2", "feed_cmd3"] })))
			.toBe(3);
	});
	...
	it("miniflare/argument-hostile-values - binds slice elements as data, never as SQL text", async () => {
		...
		const survivors = await env.DB
			.prepare("SELECT text_value FROM samples ORDER BY id")
			.all<{ text_value: string }>();
