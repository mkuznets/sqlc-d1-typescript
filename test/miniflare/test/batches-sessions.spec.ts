...
const exec = (name: string, sql: string): QueryDescriptor<void> => descriptor({
	kind: "exec",
	name,
	sql,
...
const countQuirks = (): QueryDescriptor<number | null> => descriptor({
	kind: "one",
	name: "CountQuirks",
...
const missingTable = (): QueryDescriptor<Record<string, unknown> | null> => descriptor({
	kind: "one",
	name: "MissingTable",
...
describe("native batch boundary against isolated Miniflare D1", () => {
	...
	it("miniflare/batch-post-success-mapping - maps only after native success, exposes no tuple, and cannot roll back effects", async () => {
		...
		const rows = await env.DB
			.prepare("SELECT id, flag FROM quirks WHERE id IN (3601, 3602) ORDER BY id")
			.all<{ id: number; flag: number }>();
		expect(rows.results).toEqual([{ id: 3601, flag: 1 }, { id: 3602, flag: 2 }]);
...
describe("session executors and bookmarks against isolated Miniflare D1", () => {
	it("miniflare/session-starts - accepts every starting form and exposes inherited execute and batch APIs", async () => {
		...
		for (const session of sessions) {
			...
			const [count, nothing] = await session.batch(
				countQuirks(),
				exec("SessionNoop", "UPDATE quirks SET flag = flag WHERE id = -1"),
			);
	...
	it("miniflare/session-bookmark-transfer - transfers an opaque native bookmark locally when Miniflare provides one", async () => {
		...
		if (bookmark !== null) {
			...
			const stored = await continued.execute(descriptor<Record<string, unknown> | null>({
				kind: "one",
