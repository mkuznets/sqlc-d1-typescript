...
describe('ordinary command semantics against real D1', () => {
	...
	it('miniflare/command-execlastid - :execlastid returns a rowid a follow-up read confirms', async () => {
		...
		const stored = await env.DB.prepare('SELECT text_value FROM samples WHERE id = ?').bind(second).first<{ text_value: string }>();
	...
	it('miniflare/command-execresult - :execresult hands back the native result untouched', async () => {
		...
		const survivor = await env.DB.prepare("SELECT count(*) AS total FROM samples WHERE text_value = 'native-2'").first<{ total: number }>();
	...
	it('miniflare/command-many-returning - :many over DELETE with RETURNING maps every deleted row', async () => {
		...
		const remaining = await env.DB.prepare("SELECT count(*) AS total FROM samples WHERE text_value = 'many-delete'").first<{
			total: number;
		}>();
	...
	it('miniflare/command-all-batch - resolves all six commands from one native batch, positionally', async () => {
		...
		const inserted = await env.DB.prepare('SELECT text_value FROM samples WHERE id = ?').bind(lastId).first<{ text_value: string }>();
