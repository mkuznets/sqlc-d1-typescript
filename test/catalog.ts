...
export const executableCatalog: readonly CatalogTest[] = [
  ...
  ...["value-command-metadata", "macro-smoke", "batch-success", "batch-rollback", "direct-session", "bookmark-transfer", "native-error-identity", "post-execution-result-error"].map((id) => entry(`managed-d1/${id}`, "managed-d1", "test/managed-d1/src/scenarios.ts")),
  ...
  ...["managed-protocol-size", "managed-reaper-pagination", "managed-lifecycle-failures", "managed-lifecycle-attempts", "managed-lifecycle-signal", "managed-lifecycle-timeout", "managed-lifecycle-provision-races"].map((id) => entry(`verification/${id}`, "verification", "test/managed-d1-contracts.test.ts")),
];
