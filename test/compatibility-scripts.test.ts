...
async function temporaryRoot(): Promise<string> {
  ...
  for (const path of ["verification", "package-lock.json", "test/miniflare/bun.lock", "examples/d1-worker/bun.lock", "test/miniflare/wrangler.jsonc", "examples/d1-worker/wrangler.jsonc", "scripts/install-buf.sh", "scripts/install-javy.sh"]) {
...
test("verification/compatibility-config validates the reviewed local baseline", async () => {
  ...
  assert.match(config.sqlc.knownExceptions[1], /v1\.20\.0 and v1\.24\.0 parse.*Ubuntu x64 CI cells run both current fixture corpora/);
});
...
test("verification/compatibility-mutations rejects semantic and duplicated-input drift", async () => {
  ...
  try {
    ...
    await assert.rejects(checkLocalCompatibility(config, root), /cloudflare\.compatibilityDate.*test\/miniflare\/wrangler\.jsonc/);
...
test("verification/upstream-comparison is deterministic and treats releases as review signals", async () => {
  ...
  const equal = { sqlc: "v1.31.1", workersTypes: "4.20260214.0", wrangler: "4.63.0", vitestPoolWorkers: "0.12.21", miniflare: "4.20260310.0", workerd: "1.20260310.1" };
  assert.equal(compareUpstreamCompatibility(config, equal).some(({ drift }) => drift), false);
  ...
  assert.throws(() => compareUpstreamCompatibility(config, { ...equal, sqlc: "v1.32.0-rc1" }), /malformed stable version/);
  let calls = 0;
  const fakeFetch = async (url: string): Promise<Response> => {
    ...
    const name = url.includes("workers-types") ? "workersTypes" : url.includes("vitest-pool") ? "vitestPoolWorkers" : url.split("/").slice(-1)[0];
    const key = name === "workersTypes" ? "workersTypes" : name === "vitestPoolWorkers" ? "vitestPoolWorkers" : name as keyof typeof equal;
    return new Response(JSON.stringify({ "dist-tags": { latest: equal[key] }, versions: { "99.0.0": {} } }), { status: 200 });
  };
  assert.deepEqual(await fetchUpstreamFacts(fakeFetch as typeof fetch), equal);
  assert.equal(calls, 6);
  await assert.rejects(fetchUpstreamFacts(async () => new Response(JSON.stringify({ "dist-tags": {} }), { status: 200 }) as never), /missing or malformed stable/);
  await assert.rejects(fetchUpstreamFacts(async () => new Response("not json", { status: 200 }) as never), /malformed JSON/);
  await assert.rejects(fetchUpstreamFacts(async () => new Response(JSON.stringify({ "dist-tags": { latest: "2.0.0-rc.1" } }), { status: 200 }) as never), /missing or malformed stable/);
  await assert.rejects(fetchUpstreamFacts(async () => { throw new Error("timeout"); }), /timeout/);
});
...
test("verification/sqlc-matrix reports the honest corpus selected for every sample", async () => {
  ...
  assert.deepEqual(fixturesForSqlcVersion("v1.18.0").map(({ directory }) => directory), ["test/sqlc-v1-18"]);
  for (const version of ["v1.20.0", "v1.24.0", "v1.31.1"]) assert.deepEqual(fixturesForSqlcVersion(version).map(({ directory }) => directory), ["test/miniflare", "examples/d1-worker"]);
});
...
test("matrix evidence records the executable version returned by the runner", async () => {
  ...
  try {
    ...
    const evidence = await writeCompatibilityEvidence({ output, matrixResult: { sqlcVersion: "v1.24.0", fixtures: ["test/miniflare"], knownExceptions: [], typescriptVersion: "5.9.3", candidateSha256: "a".repeat(64), cleanup: "confirmed" }, actualTools: { node: "24.12.0", npm: "11.6.2", bun: "1.3.10" } }) as { tools: { sqlc: string[] } };
    assert.deepEqual(evidence.tools.sqlc, ["v1.24.0"]);
    await assert.rejects(writeCompatibilityEvidence({ output, matrixResult: { sqlcVersion: "requested", candidateSha256: "a".repeat(64) } as never }), /actual sqlcVersion/);
...
test("verification/ci-security-contract preserves one uncredentialed publication candidate", async () => {
  ...
  assert.doesNotMatch(workflow, /pull_request_target|secrets\.|environment:|wrangler deploy|d1 execute|r2|npm publish/i);
  ...
  assert.match(workflow, /config\.sqlc\.samples\.map\(\(\{ version \}\) => \(\{ version, install: version\.replace\(\/\^v\//);
  ...
  assert.match(workflow, /github\.event_name != 'schedule' && \(github\.event_name != 'workflow_dispatch' \|\| inputs\.mode == 'verification'\)/);
  assert.match(workflow, /github\.event_name == 'workflow_dispatch' && inputs\.mode == 'drift'/);
  assert.match(workflow, /inputs\.mode == 'drift'\)\) && github\.ref == format\('refs\/heads\/\{0\}', github\.event\.repository\.default_branch\)/);
  assert.match(workflow, /set -o pipefail\s+node scripts\/check-upstream-compatibility\.mjs 2>&1 \| tee -a "\$GITHUB_STEP_SUMMARY"/);
  assert.doesNotMatch(workflow, /compatibility-evidence\.json.*publication-candidate/s);
  for (const job of ["exact-candidate", "public-types", "miniflare", "canonical-example", "generated-drift", "sqlc-compatibility"]) {
    const start = workflow.indexOf(`  ${job}:`); assert.notEqual(start, -1, job);
    const nextMatch = /^  [a-z][a-z-]+:/gm; nextMatch.lastIndex = start + `  ${job}:`.length;
  ...
  for (const version of config.sqlc.samples.map(({ version }) => version)) assert.doesNotMatch(workflow, new RegExp(`sqlc: \\[.*${version.replace(/\./g, "\\.")}`));
