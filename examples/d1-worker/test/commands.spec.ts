...
describe("ordinary command semantics against real D1", () => {
	...
	it(":execlastid returns a rowid a follow-up read confirms", async () => {
		...
		const stored = await env.DB
			.prepare("SELECT text_value FROM samples WHERE id = ?")
			.bind(second)
	...
	it(":execresult hands back the native result untouched", async () => {
		...
		const survivor = await env.DB
			.prepare("SELECT count(*) AS total FROM samples WHERE text_value = 'native-2'")
			.first<{ total: number }>();
	...
	it(":one over DML with RETURNING maps the first row and the write persists", async () => {
		...
		const both = await env.DB
			.prepare("SELECT id, title FROM feeds WHERE user_id = 'user_ret' ORDER BY id")
	...
	it(":many over DELETE with RETURNING maps every deleted row", async () => {
		...
		const remaining = await env.DB
			.prepare("SELECT count(*) AS total FROM samples WHERE text_value = 'many-delete'")
			.first<{ total: number }>();
