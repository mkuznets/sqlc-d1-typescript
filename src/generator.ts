...
export function generateValidated(validated: ValidatedGeneration): GenerateResponse {
  ...
  for (const module of plan.queryModules) {
    ...
    for (const query of module.queries) {
      ...
      if (query.argsTypeName)
        nodes.push(interfaceDecl(query.argsTypeName, query.argumentFields, (field) => driver.argumentType(field)));
      if (query.rowTypeName)
        nodes.push(interfaceDecl(query.rowTypeName, query.rowFields, (field) => driver.rowType(field)));
      if (query.parserName && query.rowTypeName)
        nodes.push(driver.parseFnDecl(query.parserName, query.rowTypeName, query.rowFields));
    ...
    if (module.runtimeValueImports.length > 0) {
      importLines.push(
        `import { generatedInternals as ${RUNTIME_VALUE_ALIAS} } from ${module.runtimeSpecifierLiteral};`,
      );
    }
    if (module.runtimeTypeImports.length > 0) {
      importLines.push(
        `import type { ${module.runtimeTypeImports.join(", ")} } from ${module.runtimeSpecifierLiteral};`,
      );
    }
