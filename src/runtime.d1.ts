...
export abstract class QueryExecutor {
  ...
  async execute<Result>(query: QueryDescriptor<Result>): Promise<Result> {
    ...
    switch (internalQuery.kind) {
      case "one": {
        ...
        return internalQuery.parse(row as Record<string, unknown>, {
          operation: "execute",
          queryName,
          rowIndex: 0,
        }) as Result;
      ...
      case "one-insert": {
        ...
        return internalQuery.parse(result.results[0] as Record<string, unknown>, {
          operation: "execute",
          queryName,
          rowIndex: 0,
        }) as Result;
  ...
  private async executeBatch<Q extends NonEmptyQueryTuple>(queries: Q): Promise<BatchResults<Q>> {
    ...
    const stmts = internalQueries.map((query) => this.executor.prepare(query.sql).bind(...query.params));
...
export class DB extends QueryExecutor {
  ...
  withSession(constraintOrBookmark?: string): SessionExecutor {
    ...
    const session = constraintOrBookmark === undefined ? db.withSession() : db.withSession(constraintOrBookmark);
