...
export function parseSemver(value, { prefixed = false } = {}) {
  if (typeof value !== "string" || (prefixed ? !value.startsWith("v") : value.startsWith("v"))) throw usageError(`rejected version ${JSON.stringify(value)}; expected ${prefixed ? "v" : ""}MAJOR.MINOR.PATCH with an optional SemVer prerelease and no build metadata`);
  const version = prefixed ? value.slice(1) : value;
  if (!SEMVER.test(version)) throw usageError(`rejected version ${JSON.stringify(value)}; expected ${prefixed ? "v" : ""}MAJOR.MINOR.PATCH with an optional SemVer prerelease and no build metadata`);
  return version;
}
export function canonicalWasmFilename(version) { return `sqlc-gen-d1-typescript_${parseSemver(version)}.wasm`; }
export function canonicalManifestFilename(version) { return `sqlc-gen-d1-typescript_${parseSemver(version)}.manifest.json`; }
...
function assertSource(sourceCommit) { if (!SOURCE_SHA.test(sourceCommit ?? "")) throw usageError(`sourceCommit must be a full 40-character lowercase SHA`); }
function assertId(value, field) { if (!ID.test(value ?? "")) throw usageError(`${field} must be a decimal string`); }
function equal(field, expected, actual) { if (JSON.stringify(expected) !== JSON.stringify(actual)) throw usageError(`${field} mismatch: expected ${JSON.stringify(expected)}, received ${JSON.stringify(actual)}`); }

export async function resolveReleaseIntent({ eventName, refType, refName, manualVersion, sourceCommit, defaultBranch, workflowRunId, serverUrl = "https://github.com", repository, isAncestor }) {
  assertSource(sourceCommit); assertId(workflowRunId, "workflowRunId");
  if (!defaultBranch || !repository || typeof isAncestor !== "function") throw usageError("defaultBranch, repository, and isAncestor are required");
  let version, tag, dryRun;
  if (eventName === "push") {
    if (refType !== "tag") throw usageError("release push must use a tag ref");
    version = parseSemver(refName, { prefixed: true }); tag = refName; dryRun = false;
  } else if (eventName === "workflow_dispatch") {
    version = parseSemver(manualVersion); tag = null; dryRun = true;
  } else throw usageError(`unsupported release event ${eventName}`);
  if (!await isAncestor(sourceCommit, defaultBranch)) throw usageError(`source commit ${sourceCommit} is not reachable from default branch ${defaultBranch}`);
  return { version, tag, sourceCommit, defaultBranch, dryRun, workflowRunId, workflowUrl: `${serverUrl.replace(/\/$/, "")}/${repository}/actions/runs/${workflowRunId}` };
}

export async function describeCandidate({ wasmPath, intent }) {
  const bytes = await readFile(resolve(wasmPath));
  return { schemaVersion: 1, plugin: PLUGIN, version: intent.version, tag: intent.tag, sourceCommit: intent.sourceCommit, workflowRunId: intent.workflowRunId, workflowUrl: intent.workflowUrl, buildPolicy: POLICY, filename: canonicalWasmFilename(intent.version), size: bytes.length, sha256: createHash("sha256").update(bytes).digest("hex") };
}
export async function writeCandidateBundle({ wasmPath, directory, intent }) {
  const descriptor = await describeCandidate({ wasmPath, intent }); const target = resolve(directory);
...
const candidateKeys = ["buildPolicy", "filename", "plugin", "schemaVersion", "sha256", "size", "sourceCommit", "tag", "version", "workflowRunId", "workflowUrl"];
export async function validateCandidateBundle({ directory, intent }) {
  let source; try { source = await readFile(resolve(directory, "candidate.json"), "utf8"); } catch { throw usageError("candidate.json is missing or unreadable"); }
  if (!source.endsWith("\n") || source.endsWith("\n\n")) throw usageError("candidate.json must have exactly one trailing newline");
  let descriptor; try { descriptor = JSON.parse(source); } catch { throw usageError("candidate.json is malformed JSON"); }
  ...
  const expected = { schemaVersion: 1, plugin: PLUGIN, version: intent.version, tag: intent.tag, sourceCommit: intent.sourceCommit, workflowRunId: intent.workflowRunId, workflowUrl: intent.workflowUrl, buildPolicy: POLICY, filename: canonicalWasmFilename(intent.version) };
  for (const [field, value] of Object.entries(expected)) equal(`candidate.${field}`, value, descriptor[field]);
  if (!SHA.test(descriptor.sha256 ?? "")) throw usageError("candidate.sha256 must be exactly 64 lowercase hexadecimal characters");
  if (!Number.isSafeInteger(descriptor.size) || descriptor.size < 1) throw usageError("candidate.size must be a positive integer");
  const files = (await readdir(resolve(directory))).sort(); equal("candidate bundle files", ["candidate.json", "candidate.sha256", descriptor.filename].sort(), files);
  const digestText = await readFile(resolve(directory, "candidate.sha256"), "utf8"); equal("candidate.sha256 file", `${descriptor.sha256}\n`, digestText);
  const wasm = resolve(directory, descriptor.filename); const retained = await readCandidate(wasm, descriptor.sha256); equal("candidate.size", descriptor.size, retained.bytes.length);
  return descriptor;
}

function expectedEvidence(config, sqlc, sha256) {
  return { candidateSha256: sha256, tools: { node: config.tools.node, npm: config.tools.npm, bun: config.tools.bun, sqlc: [sqlc], typescript: [config.typescript.floor, config.typescript.current], workersTypes: config.cloudflare.workersTypes, wrangler: config.cloudflare.wrangler, vitestPoolWorkers: config.cloudflare.vitestPoolWorkers, miniflare: config.cloudflare.miniflare, workerd: config.cloudflare.workerd, buf: config.tools.buf, javy: config.tools.javy }, configuration: { compatibilityDate: config.cloudflare.compatibilityDate, compatibilityFlags: config.cloudflare.compatibilityFlags, knownExceptions: config.sqlc.knownExceptions } };
}
export async function validateCompatibilityEvidence({ path, candidateSha256, sqlcVersion, config, root = process.cwd() }) {
  if (!SHA.test(candidateSha256 ?? "")) throw usageError("candidate SHA-256 is malformed");
  const schema = JSON.parse(await readFile(resolve(root, "verification/evidence.schema.json"), "utf8")); const validate = (await ajv()).compile(schema);
  let evidence; try { evidence = JSON.parse(await readFile(resolve(path), "utf8")); } catch { throw usageError(`compatibility evidence ${path} is malformed or unreadable`); }
  if (!validate(evidence)) throw usageError(`compatibility evidence ${path} schema mismatch: ${JSON.stringify(validate.errors)}`);
  ...
  for (const field of ["candidateSha256", "tools", "configuration"]) equal(`compatibility ${sqlc}.${field}`, expected[field], evidence[field]);
  if (evidence.cleanup.status !== "confirmed" || !evidence.scenarios.length || evidence.scenarios.some(({ status }) => status !== "passed")) throw usageError(`compatibility ${sqlc} does not record passed scenarios and confirmed cleanup`);
  return evidence;
}
...
function semanticManifest(manifest, { intent, candidate, artifactId, config, managedEvidence, managedEvidenceArtifactId } = {}) {
  ...
  if (intent) { equal("manifest.version", intent.version, manifest.version); equal("manifest.tag", intent.tag, manifest.tag); equal("manifest.dry_run", intent.dryRun, manifest.dry_run); equal("manifest.source_commit", intent.sourceCommit, manifest.source_commit); equal("manifest.workflow_url", intent.workflowUrl, manifest.workflow_url); }
  if (candidate) { equal("manifest artifact filename", candidate.filename, manifest.artifact.filename); equal("manifest artifact sha256", candidate.sha256, manifest.artifact.sha256); equal("manifest artifact size", candidate.size, manifest.artifact.size); }
...
export async function validateReleaseManifest({ manifest, path, intent, candidate, artifactId, config, managedEvidence, managedEvidenceArtifactId, root = process.cwd() }) {
  ...
  if (path) {
    let source;
    try { source = new TextDecoder("utf-8", { fatal: true }).decode(await readFile(resolve(path))); } catch { throw usageError("release manifest is unreadable or is not valid UTF-8"); }
    try { value = JSON.parse(source); } catch { throw usageError("release manifest is malformed JSON"); }
...
async function cli() {
  const [command, ...rest] = process.argv.slice(2); const values = args(rest); const output = values.output;
  if (command === "intent") {
    const intent = await resolveReleaseIntent({ eventName: values.event, refType: values["ref-type"], refName: values["ref-name"], manualVersion: values.version, sourceCommit: values.sha, defaultBranch: values["default-branch"], workflowRunId: values["run-id"], serverUrl: values["server-url"], repository: values.repository, isAncestor: gitAncestor });
