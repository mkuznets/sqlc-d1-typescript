...
export type RemoteD1Slot = { result: "not-run"; date: null; evidenceArtifactId: null } | { result: "passed" | "failed"; date: string; evidenceArtifactId: string };
...
export function createReleaseManifest(options: { intent: ReleaseIntent; candidate: CandidateDescriptor; artifactId: string; config: CompatibilityConfig; evidence: unknown[]; remote?: RemoteD1Slot }): ReleaseManifest;
export function validateReleaseManifest(options: { manifest?: ReleaseManifest | unknown; path?: string; intent?: ReleaseIntent; candidate?: CandidateDescriptor; artifactId?: string; config?: CompatibilityConfig; root?: string }): Promise<ReleaseManifest>;
export function writeReleaseManifest(options: { output: string; intent: ReleaseIntent; candidate: CandidateDescriptor; artifactId: string; config: CompatibilityConfig; evidence: unknown[]; remote?: RemoteD1Slot }): Promise<ReleaseManifest>;
