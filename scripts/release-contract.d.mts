...
export interface ReleaseManifest {
  artifact: { actions_artifact_id: string; filename: string; sha256: string; size: number; url: string };
  build_policy: "build-once-exact-artifact";
  dry_run: boolean;
  plugin: "sqlc-d1-typescript";
  remote_d1: { date: string | null; evidence_artifact_id: string | null; result: "not-run" | "passed" | "failed" };
  schema_version: 1;
  source_commit: string;
  tag: string | null;
  tested_versions: unknown;
  verification_configuration: unknown;
  version: string;
  workflow_url: string;
}
...
export function resolveReleaseIntent(options: {
  ...
  serverUrl?: string;
  repository: string;
  isAncestor: (source: string, branch: string) => boolean | Promise<boolean>;
...
export function writeCandidateBundle(options: {
  wasmPath: string;
  directory: string;
  intent: ReleaseIntent;
}): Promise<CandidateDescriptor>;
export function validateCandidateBundle(options: {
  directory: string;
  intent: ReleaseIntent;
}): Promise<CandidateDescriptor>;
export function validateCompatibilityEvidence(options: {
  path: string;
...
export function collectCompatibilityEvidence(options: {
  paths: string[];
  candidateSha256: string;
  config: CompatibilityConfig;
  root?: string;
}): Promise<unknown[]>;
export function validateCompatibilitySet(options: {
  paths: string[];
  candidateSha256: string;
  config: CompatibilityConfig;
  root?: string;
