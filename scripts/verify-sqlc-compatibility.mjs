...
export function fixturesForSqlcVersion(version) {
  return version === "v1.18.0" ? [floorFixture] : fixtures;
}

const normalizeVersion = (text) => {
  ...
  try {
    ...
    for (const fixture of selectedFixtures) {
      ...
      try {
        ...
        throw new Error(`${sqlcVersion} generation failed for ${fixture.directory}: ${error.message}`);
      }
      ...
      try {
        ...
        throw new Error(
          `${sqlcVersion} compile failed for ${fixture.directory} with TypeScript ${config.typescript.current}: ${error.message}`,
        );
      }
    }
  } catch (error) {
