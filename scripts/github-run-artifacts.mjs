...
export async function listRunArtifacts({ owner, repo, runId, name, prefix, token, fetchImpl = fetch, signal } = {}) {
  ...
  do {
    ...
    try {
      ...
      throw apiError(error instanceof Error ? error.message.replaceAll(token, "[REDACTED]") : "request failed");
    }
...
export function parseRunArtifactList(source) {
  let value;
  try {
...
async function cli() {
  ...
  const allowCreate =
    values["allow-create"] === undefined
      ? true
      : values["allow-create"] === "true"
        ? true
        : values["allow-create"] === "false"
          ? false
          : (() => {
              throw usageError("--allow-create must be true or false");
            })();
