...
export async function loadCompatibilityConfig({ root = process.cwd(), checkLocal = false } = {}) {
  ...
  const validate = new Ajv2020({ allErrors: true, strict: false, formats: { date: /^\d{4}-\d{2}-\d{2}$/ } }).compile(
    JSON.parse(schema),
  );
  if (!validate(config)) {
...
function validateSemantics(config) {
  ...
  if (samples[0].role !== "floor" || samples.at(-1).role !== "ceiling")
  ...
  if (stripV(config.sqlc.testedCeiling) !== versions.at(-1))
    fail("sqlc.testedCeiling", `must equal last sample ${samples.at(-1).version}`);
...
function actualVersion(entry) {
  return Array.isArray(entry) ? String(entry[0]).replace(/^.*@(?=\d)/, "") : undefined;
}

function equal(path, expected, actual, file) {
...
async function json(root, path, jsonc = false) {
...
export async function readLocalCompatibilityFacts(root = process.cwd()) {
  ...
  const scriptVersion = (source, file) => {
...
export async function checkLocalCompatibility(config, root = process.cwd()) {
  ...
  for (let index = 0; index < facts.fixtureFacts.length; index++)
    for (const field of ["wrangler", "vitestPoolWorkers", "miniflare", "workerd"])
...
export function renderCompatibilityFacts(config) {
  const rows = [
    ...
    ...["workersTypes", "wrangler", "vitestPoolWorkers", "miniflare", "workerd", "compatibilityDate"].map((key) => [
      `cloudflare.${key}`,
