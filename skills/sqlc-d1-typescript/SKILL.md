...
2. **Check the tag match.** Read the `INSTALLED_TAG` file beside this skill. This skill's guidance describes one
   Plugin version. When `INSTALLED_TAG` is absent or does not read `v<version>`, stop, report the mismatch, and
   give the human the install command for the matching tag:
...
3. **Copy the two values together.** Open the release record for `v<version>` — the GitHub release for that tag
...
Version-specific facts — the supported sqlc range, the TypeScript evidence, the exact Cloudflare baseline —
come from that release record and from
...
the same release record, and `INSTALLED_TAG` reads `v<version>`.
...
holds `runtime.ts` plus one `<source>_sql.ts` per SQL source file. Those files are outputs of the pinned bytes:
their provenance is the artifact at the configured URL, which sqlc checked against the configured SHA-256
before executing it. They open with a do-not-edit notice, and they stay byte-identical to what the Plugin
wrote. When the output has to change, change the schema, the query, or the configuration and generate again.
...
configuration is what changes; regenerate and typecheck again. Make the generated types right, and keep
consumer code free of type assertions and compiler suppressions. Generated files stay as the Plugin wrote them.
